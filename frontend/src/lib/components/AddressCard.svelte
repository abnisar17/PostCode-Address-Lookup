<script lang="ts">
	...
	let hasPricePaid = $derived((address.price_paid?.length ?? 0) > 0);
	let hasCompanies = $derived((address.companies?.length ?? 0) > 0);
	let hasFoodRatings = $derived((address.food_ratings?.length ?? 0) > 0);
	let hasEnrichment = $derived(hasPricePaid || hasCompanies || hasFoodRatings);
			...
			{#if hasCompanies}
				<details class="group rounded border border-gray-100">
					...
					>
						...
						Companies
						<span class="ml-auto text-xs font-normal text-gray-400"
							>{address.companies?.length}
							{address.companies?.length === 1 ? 'company' : 'companies'}</span
						>
					</summary>
					<div class="border-t border-gray-100 px-3 py-2">
						<ul class="space-y-1.5">
							{#each address.companies ?? [] as company (company.id)}
								<li class="text-sm">
			...
			{#if hasFoodRatings}
				<details class="group rounded border border-gray-100">
					...
					>
						...
						Food Ratings
						<span class="ml-auto text-xs font-normal text-gray-400"
							>{address.food_ratings?.length}
							{address.food_ratings?.length === 1 ? 'rating' : 'ratings'}</span
						>
					</summary>
					<div class="border-t border-gray-100 px-3 py-2">
						<ul class="space-y-2">
							{#each address.food_ratings ?? [] as rating (rating.id)}
								<li class="text-sm">
